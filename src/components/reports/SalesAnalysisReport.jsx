...
const SalesAnalysisReport = ({ transactions }) => {
  ...
  const computeSalesAnalysis = () => {
    ...
    filtered.forEach(t => {
      rows.push({
        ...
        'Total Sales': (t.total_amount || 0).toFixed(2)
      });
  ...
  const headers = ['Date', 'Product Name', 'Size', 'Base', 'Quantity sold', 'Total Sales'];
  const exportAnalysis = () => exportToExcel(`sales_analysis_${startDate}_to_${endDate}.xlsx`, headers, analysisRows, formatCurrency(analysisTotal));
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-4">
          <div>
            <h4 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-white">Sales Analysis</h4>
            <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Sales data for the selected date range</p>
          </div>
        ...
        {/* Mobile view uses renderTable, desktop uses original table */}
